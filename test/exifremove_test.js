...
function checkForExifHeader(buffer) {
    return checkForAPP1Header(buffer, [(buffer, i) => {
        return (buffer[i + 4].toString(16) + buffer[i + 5].toString(16) === "4578"
...
describe("exifremove", function() {
    ...
    describe("remove", function() {
        ...
        it("should keep APP1 section if keepMarker is specified", function() {
            ...
            assert.equal(checkForAPP1Header(result, [(buffer, i) => {
    ...
    describe("removeMultiple", function() {
        it("should remove EXIF from all images passed in", function() {
            var result = exifremove.removeMultiple([imageBuffer, fs.readFileSync("test/img/test2.jpg")]);
